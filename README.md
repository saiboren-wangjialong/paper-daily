...
会议源会先从 DBLP 获取题录、作者、DBLP 链接以及可用的 DOI/出版社链接。对筛选后的会议论文，系统会再按标题依次查询 arXiv、Semantic Scholar、OpenAlex 和 Crossref；如果标题能可靠匹配且来源提供摘要，就回填摘要、PDF 链接和分类，再进入中文分析流程。没有找到摘要的会议论文不会默认让模型凭标题猜创新点。
...
| `ARXIV_EXPAND_CATEGORY_SEARCH` | `true` | arXiv 除关键词精确查询外，再按相关分类抓最近论文，让 medium/low 候选更容易进入筛选 |
| `ARXIV_CATEGORY_MAX_RESULTS` | `10` | 每个方向额外按 arXiv 分类抓取的最大数量 |
| `MAX_CONFERENCE_ABSTRACT_ENRICHMENTS` | `50` | 每次最多对多少篇会议候选论文按标题补摘要 |
| `CONFERENCE_ABSTRACT_SOURCES` | `arxiv,semantic_scholar,openalex,crossref` | 顶会论文摘要回填来源顺序 |
